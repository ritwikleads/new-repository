...
app.post('/process-address', async (req, res) => {
    ...
    try {
        ...
        if (expandMapButton) {
            ...
            try {
                await new Promise(resolve => setTimeout(resolve, 1500)); // Changed to 1.5 seconds
            } catch (err) {
        ...
        // Capture a simple screenshot of the Google Maps window with a 1.5-second delay

        // Select the Google Maps element
        const mapElementSelector = 'div.gm-style';
        await page.waitForSelector(mapElementSelector, { timeout: 500 });
        const mapElement = await page.$(mapElementSelector);

        let screenshotBase64 = null;

        if (mapElement) {
            // Take a screenshot of the map element only
            screenshotBase64 = await mapElement.screenshot({
                encoding: 'base64' // Get the screenshot as a Base64-encoded string
            });
            ...
            throw new Error("Map element not found for screenshot");
        }
